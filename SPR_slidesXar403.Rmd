---
title: "RMarkdown"
subtitle: "Introduction to <br> Reproducible Reporting"
author: "Steve Riley, Pfizer Inc"
date: "`r Sys.Date()`"
output: 
  xaringan::moon_reader:
   incremental: true
   widescreen: true
   logo: "images/Pfizer-Logo-Color-RGB.png"
   css: ["css/my-theme.css", "css/my-fonts.css", "css/styles.css"]
   seal: false
   lib_dir: libs
   nature:
     ratio: '16:9'
...
<div style = "position: absolute;top: -50px;right: -200px;"><img src="https://images.unsplash.com/photo-1603204254626-d0de8eb24cf1?ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&ixlib=rb-1.2.1&auto=format&fit=crop&w=1471&q=80" alt="A collection of wooden letters loosely arranged. The letters are used for stamps." height="800" width="900"></img></div>
...
### Many files and graphics used in these slides are [on Github](https://github.com/jthomasmock/rmd-nhs/tree/master/workshop)

### Slides **heavily** borrowed from a [presentation](https://jthomasmock.github.io/rmd-nhs/) by Tom Mock from RStudio
...
No more copy-paste, no more manually rebuilding analyses from disparate components, no more dread when the data are updated and you need to re-run an analysis.
...
```{r, echo = FALSE, warning=FALSE}
paste_table(sections = c("Documents", "Presentations"))
```
...
You can write inline R code to programatically generate specific components of the document.

--

i.e.,  I can report that there are `r knitr::inline_expr("nrow(mtcars)")` 
...
````md
knitr::include_graphics("path-to-graphic.png")
````
...
Code output in a chunk auto prints, unless saved to an object, which needs to be explicitly returned.
...
```{r plotEx, fig.dim = c(4,6), dpi = 150, eval = TRUE, echo = FALSE}
```
...
.pull-right[
````md
```{r myPlt, dpi=300,fig.dim=c(6,4), eval = TRUE}`r ''`
```
````
...
`r knitr::include_graphics("images/hourglass.png")`
...
- Click `+C` (Insert new chunk button)  
...
```{r surv-plot, eval = TRUE, fig.retina=3, echo=FALSE}
```
...
- [`gtExtras`](https://jthomasmock.github.io/gtExtras/): Tom Mock's package, primarily for embedding many data visualizations into `gt` tables.
...
The `gtsummary` package coupled with `gt` is especially powerful as it combines common tasks with rich support for outputs (Word, PDF, HTML). It is a wrapper around several packages with many additional capabilities built in.
...
```{r, regression tables,  warning=FALSE}
t1 <-
...
Also, Markdown is portable. Files containing Markdown-formatted text can be opened using virtually any application. [Markdown Guide](https://www.markdownguide.org/getting-started/)
...
Useful for scientific writing native to the web
...
- [Advanced Rmarkdown - Yihui](https://slides.yihui.org/2019-rstudio-conf-rmarkdown-workshop.htm#22)
